...
# Esto no es un programa complejo, pero es entendible que pueda complicarse cuando estás aprendiendo. Por más que lo que has aprendido hasta ahora es muy simple, ponerlo todo junto en un solo programa puede ser complejo, por lo que te doy un par de ayudas: Este programa debería comenzar preguntando cosas al usuario, por lo tanto, #* vas a necesitar input para poder recibir los ingresos del usuario y deberías usar variables para almacenar esos ingresos. Recuerda que los ingresos de usuarios se almacenan como strings. Por lo tanto, deberías convertir uno de esos ingresos en un float para poder hacer operaciones con él.
...
#- todo eso en un string al que debes dar formato. Recuerda que conocimos dos maneras de hacerlo y cualquiera de ellas es válida. No dejes de intentar resolverlo, y si se te complica, en la próxima lección lo hacemos juntos.

#todo... En el archivo Solucion2.py estará la solución que yo he encontrado y aunque puede ser diferente a la tuya, lo único que importa es que lo hayas intentado y si es diferente a la mía pero funciona... ¡¡Felicidades!!